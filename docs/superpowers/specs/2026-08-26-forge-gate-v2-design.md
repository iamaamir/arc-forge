...
3. **Grill**: challenge the user on each rule and each undecided boundary. One question per message — never batch questions. When the host harness provides a structured-question tool (option lists with free-text fallback, e.g. opencode's `question` tool), it MUST be used; otherwise render the same structure in text. Every question MUST offer 2–4 concrete options grounded in the analysis (each with its trade-off stated in one line) plus the free-text escape hatch. The agent never asks an open-ended question it could have offered options for, and never presents options without having a recommendation marked. Each answer may open follow-up questions; the session continues until every branch of the decision tree is resolved. Unresolved questions block finalization.
