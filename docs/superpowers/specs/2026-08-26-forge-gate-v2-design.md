...
3. **Grill**: challenge the user on each rule and each undecided boundary. One question per message — never batch questions. Every question MUST be presented as 2–4 concrete options grounded in the analysis (each with its trade-off stated in one line), plus an explicit "or describe your own" escape hatch — the user either picks one or types their own answer. The agent never asks an open-ended question it could have offered options for, and never presents options without having a recommendation marked. Each answer may open follow-up questions; the session continues until every branch of the decision tree is resolved. Unresolved questions block finalization.
